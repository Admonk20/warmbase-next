...
async function main() {
  ...
  if (!userId) {
    console.error('Usage: node scripts/delete_user_by_id.mjs <userId>');
    process.exit(1);
  }

  const res = await requestJson('DELETE', `${url}/auth/v1/admin/users/${userId}`, null, { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` });
  console.log('status', res.status);
  if (res.body) console.log(JSON.stringify(res.body));
}
